...
A **koi fish** that glides calmly around the pond (water only — it never crosses the shoreline) and pops up to the surface every few seconds with a ripple and a soft "bloop", faint and teal-tinted when deep, bright with a wet back-glint when it surfaces. The ducks completely ignore it (`spooks:false`).
...
- **Koi** — `this.life=rand(20,34)` is how long it stays; glide speed is `maxSpeed=20+...` and `steer=dt*1.8` in `Koi.prototype.update`; how often it surfaces is `this.nextSurf=rand(3.5,7.5)` and the dwell is `this.surfDur=rand(1.3,2.4)`. Colour variants (white-kohaku vs all-orange) are the `this.pal` ternary in `Koi()`.
