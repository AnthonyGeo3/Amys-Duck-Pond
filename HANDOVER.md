...
- **Random visitors**: every 3-5 minutes (randomised, one at a time) a cute guest wanders in. Built on an extensible visitor framework so more animals can be added; so far there's a **ginger cat** that strolls in from the grass, play-stalks and pounces at the ducks (who wake, bunch into a loose huddle and scatter at each pounce, then regroup), meows now and then, and saunters off after 20-30s. When he pounces the flock scatters in all directions like startled headless chickens, then drifts back together. On land he's quick enough to get a playful "boop" in; if the ducks flee onto the water he'll wade after them but moves slower there, so they can paddle clear. The ducks ignore food while he's around.
...
- **`audio` module** — a self-contained IIFE using the Web Audio API. All sound is synthesised live (no audio files): a breeze bed, an airy shimmer, occasional birdsong, plus one-shot sounds (pellet plip, eat nom, happy chirp, and a cat meow). All one-shots no-op unless sound is enabled.
...
- **Visitor frequency** — `var VISITOR_MIN=180, VISITOR_MAX=300;` near the top (seconds between guests = 3-5 min; drop these to, say, `8`/`15` to see visitors quickly while testing). Which animals can appear is the `VISITORS` array. Cat visit length is `this.life=rand(20,30)` in `Cat()`; its stalk/pounce timings, speeds and crouch are in `Cat.prototype.update`. How slow he is in water is the `*0.40` multiplier + the `0.92` drag + the `80` speed cap in `Cat.prototype.update` (raise the cap to let him keep up more). The panic scatter is `spookFlock()`: `panicT=rand(0.7,1.4)` is how long they scarper, `panicAng=away±1.0` how randomly they fan out (widen the range for more chaos), and the duck `huddle` branch runs them at `RUN*0.95`. The pounce distance that sets off the scatter is the `<58` test in the cat's pounce; how far the cat dives in is `stopAt:16`. Meow cadence is `this.meowT=rand(4.5,9.5)`.
...
- **More visitors** (the cat framework is built and ready — each is a new class added to `VISITORS`): koi fish that surface in the water only, a strolling tortoise that climbs out then back in, a frog the ducks actually chase (inverts the cat logic — ducks approach instead of flee), dragonflies that flit over the pond, and a red panda ambling along the grass.
