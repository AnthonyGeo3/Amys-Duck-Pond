...
- **Dragonflies** — count is the `this.flies=[makeDfly(), makeDfly()]` in `Dragonfly()`; visit length `this.life=rand(18,30)`. Dart speed is `maxSpeed=140` (wander) / `195` (fleeing a duck) in `updateDfly`; how easily a duck takes the bait is the `<64` range + `<dt*1.6` roll in the `lures` branch, and how long it chases is `chaseT=rand(2.5,5.5)`. Colours are the `col` ternary in `makeDfly()`. When chased, a fly flees away from the duck plus a wall-repulsion (the `M=90` / `*1.5` term in `updateDfly`) that deflects it along edges so it heads to open water instead of a corner; on `leaving` it rises off the top (`ty=-110`, with the `yLo` clamp letting it exit).
