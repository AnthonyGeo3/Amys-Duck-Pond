...
- **Visitor frequency** — `var VISITOR_MIN=180, VISITOR_MAX=300;` near the top (seconds between guests = 3-5 min; drop these to, say, `8`/`15` to see visitors quickly while testing). Which animals can appear is the `VISITORS` array. Cat visit length is `this.life=rand(20,30)` in `Cat()`; its stalk/pounce timings, speeds and crouch are in `Cat.prototype.update`. How slow he is in water is the `*0.32` multiplier + the `0.9` drag + the `72` speed cap in `Cat.prototype.update`. How far/hard the ducks bolt is `push=(state.scare||0)*78` in `manageVisitor` plus the `state.scare` radius multiplier in the duck `huddle` branch; the near-miss distance that triggers the bolt is the `<42` test in the cat's pounce. Meow cadence is `this.meowT=rand(4.5,9.5)`.
