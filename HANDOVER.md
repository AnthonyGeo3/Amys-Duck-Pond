...
- **Random visitors**: every 5-10 minutes (randomised, one at a time) a cute guest wanders in. Built on an extensible visitor framework so more animals can be added; so far there's a **ginger cat** that strolls in from the grass, play-stalks and pounces at the ducks (who wake, bunch into a huddle and scatter at each pounce, then regroup), and saunters off after 20-30s. The cat never enters the water; the ducks ignore food while it's around.
...
- **Visitor frequency** — `var VISITOR_MIN=300, VISITOR_MAX=600;` near the top (seconds between guests; drop these to, say, `8`/`15` to see visitors quickly while testing). Which animals can appear is the `VISITORS` array. Cat visit length is `this.life=rand(20,30)` in `Cat()`; its stalk/pounce timings, speeds and crouch are in `Cat.prototype.update`. How hard the ducks scatter is the `state.scare` multipliers in the duck `huddle` branch.
