...
A **stork** that glides in from the sky, lands at the water's edge, fishes for a bit — dipping its long beak down to the water with little ripples each time — then takes off and flies away. It has two drawn poses (wings-spread flight vs. tall standing/pecking). The ducks ignore it (neutral — no `spooks`/`befriends`).
...
- **Dragonflies** — count is the `this.flies=[makeDfly(), makeDfly()]` in `Dragonfly()`; visit length `this.life=rand(18,30)`. Dart speed is `maxSpeed=140` (wander) / `195` (fleeing a duck) in `updateDfly`; how easily a duck takes the bait is the `<64` range + `<dt*1.6` roll in the `lures` branch, and how long it chases is `chaseT=rand(2.5,5.5)`. Colours are the `col` ternary in `makeDfly()`.
- **Stork** — how long it fishes is `this.life=rand(13,22)` in `Stork()`; flight speed is the `maxSpeed=120/130` in `flyIn`/`flyOut`; peck cadence is `this.peckT=rand(1.1,2.6)` and each dip lasts `0.55`s (the `/0.55` in `peck`). Landing spot is `landX=shoreX(landY)-5` (right at the water's edge).
